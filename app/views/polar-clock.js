...
export default Ember.View.extend({
	didInsertElement: function() {
		...
		function tick() {
		  ...
		  field.select("text")
		      .attr("dy", function(d) { return d.value < 0.5 ? "-.5em" : "1em"; })
		...
		function fields() {
		  ...
		  return [
		    ...
		    {index: 0.5, text: formatHour(now),   value: now.getHours() / 24},
		    {index: 0.3, text: formatDay(now),    value: now.getDay() / 7},
		    {index: 0.2, text: formatDate(now),   value: (now.getDate() - 1) / (32 - new Date(now.getYear(), now.getMonth(), 32).getDate())},
		    {index: 0.1, text: formatMonth(now),  value: now.getMonth() / 12}
		  ];
		}
